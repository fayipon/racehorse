...
describe('race model', () => {
  it('ships the native preview race Godot runs without React', () => {
    expect(preview).toEqual(JSON.parse(JSON.stringify(racePlan(61293, 1, 8))))
  })
  it('keeps the field together and settles positions gradually', () => {
    for (let round = 1; round <= 60; round++) {
      let previousGap = 0
      for (let t = 1; t <= 44.5; t += .1) {
        const p = racePositions(31337, round, t, 8)
  ...
  it('often ends in a fight, sometimes a clear win', () => {
    const margins = Array.from({ length: 200 }, (_, i) => {
      const order = raceOrder(777, i + 1, 8)
      return lengths(1 - racePositions(777, i + 1, 44.5, 8)[order[1] - 1])
    })
