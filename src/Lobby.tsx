...
export default function Lobby() {
  ...
  return <>
    <header className="app-header lobby-header"><a className="brand" href={routePath(locale)} aria-label={m.lobby.brandLabel}><span className="brand-icon">♞</span><span>{m.lobby.brand[0]}<span className="brand-light">{m.lobby.brand[1]}</span><small>{m.lobby.brandNote}</small></span></a>
    ...
    <main className="app-main lobby">
      ...
      <ul className="cup-list">{(Object.keys(CUPS) as CupId[]).map(id => {
        const cup = CUPS[id]
        return <li key={id}><a className={`cup-card cup-${id}`} href={routePath(locale, id)}>
