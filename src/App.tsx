...
import { BET_CLOSE_MS, bettingOpen, BET_MS, CUPS, HORSES, cameraShot, countdown, fieldOf, odds, phaseAt, pickLabel, raceOrder, racePlan, racePositions, wins, type CupId, type Game, type Pick } from './game'
...
export function RaceStage({ game, now, muted, paused = false, children, notification, betsPanel, celebration, chatFeed, expanded = false, onCollapse, onReady }: { game: Game; now: number; muted: boolean; paused?: boolean; children?: React.ReactNode; notification?: React.ReactNode; betsPanel?: React.ReactNode; celebration?: React.ReactNode; chatFeed?: React.ReactNode; expanded?: boolean; onCollapse?: () => void; onReady: (ready: boolean) => void }) {
  ...
  return <div ref={shell} className={`race-stage phase-${phase} ${!ready ? 'is-loading' : ''} ${assembling ? 'is-assembling' : ''} ${finishing ? 'has-finished' : ''} ${cinematic ? 'is-cinematic' : ''} ${winnerCutIn ? 'is-cut-in' : ''} ${expanded ? 'is-expanded' : ''}`} style={{ '--runners': field, '--half': field / 2 } as CSSProperties}>
    {!ready && <div className="engine-loading" role="status" aria-live="polite"><div className="loading-emblem" aria-hidden="true">♞</div><span className="loading-brand">{CUPS[game.cup].en}</span><h2>{failed ? '賽場暫時無法載入' : `正在準備${CUPS[game.cup].name}賽場`}</h2><p>{failed ? '連線可能中斷或載入逾時，請重試。' : building >= 1 ? '正在準備賽場畫面…' : progress >= 100 ? '正在建立賽道與小馬…' : '正在下載小馬與賽場資源…'}</p><div className="loading-progress" role="progressbar" aria-label="賽場載入進度" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.floor(loaded)}><i style={{ width: `${loaded}%` }} /></div><strong>{Math.floor(loaded)}<small>%</small></strong><small>首次載入約 13 MB · 準備完成後開放操作</small>{failed && <button onClick={() => { setFailed(false); setProgress(0); setBuilding(0); setWarmup(0); setReady(false); onReady(false); setLoadAttempt(value => value + 1); if (frame.current) frame.current.src = gameSource(game.cup, field) }}>重新載入賽場</button>}</div>}

    <iframe tabIndex={ready ? 0 : -1} ref={frame} title="Godot 3D 即時賽馬" src={gameSource(game.cup, field)} allow="autoplay; fullscreen" className={ready ? 'ready' : ''} />
    <div className="stage-vignette" />
    {chatFeed}
    <div className="race-title"><div>RACE <em>{String(game.round).padStart(2, '0')}</em></div><span /><p>{CUPS[game.cup].en}<small>{CUPS[game.cup].name} · 1200 M</small></p></div>
    ...
    {notification}
    <button className="fullscreen" aria-label="離開全螢幕" onClick={() => { if (document.fullscreenElement) void document.exitFullscreen().catch(() => {}); else onCollapse?.() }}><Minimize2 size={16} /></button>
  </div>
}
...
export default function App({ cup: cupId }: { cup: CupId }) {
  ...
  const chat = useRaceChat(game, now)
  ...
  const winnerOdds = odds('horse:1', field).toFixed(2)
  useEffect(() => { document.title = `${cup.en} ${cup.name}・小馬競速俱樂部` }, [cup])
  ...
  return <>
    <header className="app-header"><a className="brand" href={import.meta.env.BASE_URL} aria-label="回到賽事大廳"><span className="brand-icon">♞</span><span>{cup.en.replace(/ CUP$/, '')}<span className="brand-light">CUP</span><small>小馬競速俱樂部</small></span></a><nav><a className="nav-active" href={import.meta.env.BASE_URL}><Flag size={16} />賽事大廳</a><button onClick={() => setModal('history')}><History size={16} />投注紀錄</button><button onClick={() => setModal('rules')}><CircleHelp size={16} />玩法說明</button></nav><div className="header-wallet"><span className="coin-icon"><Coins size={17} /></span><div><small>我的籌碼</small><strong>{fmt(game.balance)}</strong></div><span className="practice-label">練習模式</span></div></header>
    <main className="app-main"><div className="page-heading"><div><span className="overline">THE {cup.en} EXPERIENCE</span><h1>每一場，都有新的可能<span>。</span></h1></div><p><span className="online-dot" />{cup.venue}開放中 <span className="divider">/</span> 每 2 分鐘一場</p></div>
      <div className="game-layout"><section className="main-column">
        <div className="race-card"><div className="race-toolbar"><div><span className="live-tag"><Radio size={12} />LIVE</span><b>{cup.name}</b><span className="toolbar-detail">{cup.en} · {field} 匹 · 1200 公尺</span></div><div className="toolbar-actions"><button className="icon-button" onClick={event => enterFullscreen(event.currentTarget.closest('.race-card')?.querySelector('.race-stage'))} aria-label="賽場全螢幕"><Maximize2 size={16} /></button><button className="icon-button" onClick={toggleSound} aria-label={muted ? '開啟賽場轉播與觀眾聲' : '關閉賽場轉播與觀眾聲'}>{muted ? <VolumeX size={17} /> : <Volume2 size={17} />}</button></div></div>
        <RaceStage onReady={setGameReady} game={game} now={now} muted={muted} expanded={expanded} onCollapse={() => setExpanded(false)} chatFeed={phone && <StageChatFeed messages={chat.messages} now={now} />} celebration={phase === 'result' && result && result.payout > 0 ? <div className="payout-celebration" key={`payout-${game.round}`} role="status"><div className="payout-sparks" aria-hidden="true">{Array.from({ length: 12 }, (_, i) => <i key={i} style={{ '--spark': i } as CSSProperties} />)}</div><span className="payout-kicker">WIN! · 中獎啦</span><strong><Coins size={24} />+{fmt(result.payout)}</strong><span>派彩含本金 · 已加入籌碼</span><small>本局淨盈虧 {result.payout - result.stake >= 0 ? '+' : ''}{fmt(result.payout - result.stake)}</small></div> : null} betsPanel={<section aria-label="本局投注" className={`stage-bets placed-bets ${phase === 'result' && result && result.payout > 0 ? 'has-payout' : ''}`}><div className="ticket-strip-heading"><h3>本局投注 <span>{game.bets.length}</span></h3><b>{fmt(totalStake)}<Coins size={12} /></b></div>{game.bets.length === 0 ? <p className="empty-bets">尚無注單，選匹小馬一起加油吧。</p> : <ul>{game.bets.map(b => <li key={`${b.id}-${phase}`} className={phase === 'result' && result && wins(b.pick, result.winner, field) ? 'winning-bet' : ''}><span>{pickLabel(b.pick)}<small>{fmt(b.amount)} × {odds(b.pick, field).toFixed(2)}</small></span>{phase === 'result' && result ? <b className={wins(b.pick, result.winner, field) ? 'ticket-payout' : 'muted'}>{wins(b.pick, result.winner, field) ? <><span><Check size={10} />中獎</span><strong>+{fmt(Math.round(b.amount * odds(b.pick, field)))}</strong><small>派彩含本金</small></> : '未中獎'}</b> : <button disabled={locked} onClick={() => void cancel(b.id)} aria-label={`撤回 ${pickLabel(b.pick)} 投注`}><X size={14} /></button>}</li>)}</ul>}</section>} notification={message && <div key={message} className="toast" role="status" aria-live="polite"><span>{message}</span><button aria-label="關閉提示" onClick={() => setMessage('')}><X size={15} /></button></div>}>{dockOpen ? <div className="stage-betting"><button className="dock-toggle" onClick={() => setDockOpen(false)} aria-expanded="true">收起<ChevronDown size={14} /></button><section className="markets"><div className="section-heading"><h2><span className="section-number">01</span>選擇你的好運</h2><span>{locked ? <><LockKeyhole size={13} />本場已封盤</> : '所有馬匹機率相同'}</span></div>
          <div className="horse-grid">{HORSES.slice(0, field).map(h => <button key={h.id} className={`horse-card ${selected === `horse:${h.id}` ? 'selected' : ''}`} style={{ '--horse': h.color } as CSSProperties} disabled={locked} onClick={() => pick(`horse:${h.id}`)} aria-pressed={selected === `horse:${h.id}`}><HorseNumber id={h.id} small /><HorsePortrait id={h.id} /><span className="horse-card-name">{h.name}</span><span className="horse-odds">× {winnerOdds}</span>{selected === `horse:${h.id}` && <span className="selection-check"><Check size={10} /></span>}</button>)}</div><div className="stage-side-picks" role="group" aria-label="大小單雙投注">{(['big', 'small', 'odd', 'even'] as const).map(v => <button key={v} className={selected === v ? 'selected' : ''} disabled={locked} onClick={() => pick(v)} aria-pressed={selected === v}><b>{{ big: '大', small: '小', odd: '單', even: '雙' }[v]}</b><span>{{ big: `${half + 1}–${field}`, small: `1–${half}`, odd: listed(numbers(1)), even: listed(numbers(2)) }[v]}</span><em>× 1.90</em></button>)}</div>
          <p className="market-hint"><CircleHelp size={12} />選擇項目與籌碼，確認後完成投注。賠率含本金。</p></section><div className="stage-wager"><span>{selected ? pickLabel(selected) : '選擇你的投注項目'}<small>餘額 {fmt(game.balance)} · 已投注 {fmt(totalStake)}</small></span><div className="stage-chips" role="group" aria-label="快速選擇投注籌碼">{[100, 500, 1000].map(n => <button key={n} type="button" disabled={locked || pending} aria-pressed={amount === n} className={amount === n ? 'active' : ''} onClick={() => setAmount(n)}>{fmt(n)}</button>)}</div><label htmlFor="stage-amount">籌碼<input id="stage-amount" type="number" min="10" max="10000" step="10" value={Number.isNaN(amount) ? '' : amount} onChange={e => setAmount(e.currentTarget.valueAsNumber)} /></label><button className="place-bet" disabled={locked || !selected || pending || !Number.isSafeInteger(amount) || amount < 10 || amount > 10000 || amount % 10 !== 0 || amount > game.balance} onClick={() => void submit()}>{pending ? '處理中…' : '確認投注'}<ArrowUpRight size={16} /></button></div></div> : <button className="dock-open" onClick={() => setDockOpen(true)} aria-expanded="false">我要投注<ChevronUp size={16} /></button>}</RaceStage>
      ...
      {(!phone || needsRefill) && <aside className="chat-sidebar">{!phone && <RaceChat chat={chat} round={game.round} />}{needsRefill && <button className="chat-refill" onClick={() => void refill()}>領取 10,000 練習籌碼 <ArrowDownLeft size={13} /></button>}</aside>}</div>
      <footer><span>♞ {cup.en} <span>{cup.motto}</span></span><span>虛擬籌碼 · 純粹好玩 <span className="footer-dot">•</span> 本機自動儲存 <span className="footer-dot">•</span> <a href={`${import.meta.env.BASE_URL}audio/crowd/LICENSE.txt`} target="_blank" rel="noreferrer">觀眾音效：Gregor Quendel · CC BY 4.0</a></span></footer>
    </main>

    {modal === 'rules' && <Dialog title={`${cup.name}・玩法說明`} onClose={() => setModal(null)}><div className="rules-content"><p>每場共 {field} 匹小馬，每 120 秒自動開始新的一輪。</p><ol><li><b>投注 48 秒</b>：選擇項目與籌碼，確認後扣款，封盤前可撤回。接著集合與倒數 12 秒，期間禁止下注與撤單。</li><li><b>比賽 50 秒</b>：停止投注與撤單，觀看小馬競速。</li><li><b>結算 10 秒</b>：自動派彩，再次開放下一輪投注。</li></ol><table><thead><tr><th>玩法</th><th>中獎條件</th><th>賠率</th></tr></thead><tbody><tr><td>猜冠軍</td><td>選中第 1 名馬號</td><td>{winnerOdds}</td></tr><tr><td>猜單雙</td><td>單 {numbers(1).join('/')}；雙 {numbers(2).join('/')}</td><td>1.90</td></tr><tr><td>猜大小</td><td>小 1–{half}；大 {half + 1}–{field}</td><td>1.90</td></tr></tbody></table><p>賠率含本金。例如投注 100 籌碼、賠率 {winnerOdds}，中獎派彩 {fmt(Math.round(100 * odds('horse:1', field)))}，淨獲利 {fmt(Math.round(100 * odds('horse:1', field)) - 100)}。每匹馬的冠軍機率相同，賽果不受投注影響。</p><p className="muted">初始 10,000 虛擬籌碼；最低投注 10，最高 10,000，每筆須為 10 的倍數，每輪最多 100 筆。餘額少於 10 且無當輪注單時可免費補充。資料儲存於此瀏覽器；無儲值、提領或真實金錢交易。</p></div></Dialog>}
    {modal === 'history' && <Dialog title="賽果與投注紀錄" onClose={() => setModal(null)}><div className="history-list">{game.history.length === 0 ? <div className="history-empty"><History size={36} /><h3>第一場故事，還在路上。</h3><p>比賽結束後，這裡會顯示賽果與你的派彩。</p></div> : game.history.map(r => <div className="history-row" key={r.round}><span>#{String(r.round).padStart(2, '0')}</span><HorseNumber id={r.winner} /><div><b>{HORSES[r.winner - 1].name}</b><small>{r.winner % 2 ? '單' : '雙'} · {r.winner <= half ? '小' : '大'}　投注 {fmt(r.stake)}</small></div><strong className={r.payout >= r.stake ? 'positive' : 'negative'}>{r.payout >= r.stake ? '+' : ''}{fmt(r.payout - r.stake)}<small>淨盈虧</small></strong></div>)}<p className="muted">保留最近 12 場賽事紀錄</p></div></Dialog>}
  </>
}
