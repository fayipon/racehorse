...
export function RaceStage({ game, now, muted, paused = false, children, notification, betsPanel, celebration, onReady }: { game: Game; now: number; muted: boolean; paused?: boolean; children?: React.ReactNode; notification?: React.ReactNode; betsPanel?: React.ReactNode; celebration?: React.ReactNode; onReady: (ready: boolean) => void }) {
  ...
  return <div ref={shell} className={`race-stage phase-${phase} ${!ready ? 'is-loading' : ''} ${assembling ? 'is-assembling' : ''} ${finishing ? 'has-finished' : ''} ${cinematic ? 'is-cinematic' : ''}`}>
    ...
    {finishing && <div key={`finish-${game.round}`} className="finish-sequence" aria-live="polite"><div className="finish-announcement"><span className="finish-kicker">FIRST ACROSS THE LINE · 率先衝線</span><strong>{HORSES[order[0]-1].name}</strong><div><HorseNumber id={order[0]} /><span>本場冠軍<small>{HORSES[order[0]-1].en}</small></span><Trophy size={22} /></div></div></div>}
