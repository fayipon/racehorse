...
export default function App({ cup: cupId }: { cup: CupId }) {
  ...
  return <>
    <header className="app-header"><a className="brand" href={import.meta.env.BASE_URL} aria-label="Sunny Cup 首頁"><span className="brand-icon">♞</span><span>SUNNY<span className="brand-light">CUP</span><small>小馬競速俱樂部</small></span></a><nav><a className="nav-active" href={import.meta.env.BASE_URL}><Flag size={16} />賽事大廳</a><button onClick={() => setModal('history')}><History size={16} />投注紀錄</button><button onClick={() => setModal('rules')}><CircleHelp size={16} />玩法說明</button></nav><div className="header-wallet"><span className="coin-icon"><Coins size={17} /></span><div><small>我的籌碼</small><strong>{fmt(game.balance)}</strong></div><span className="practice-label">練習模式</span></div></header>
    <main className="app-main"><div className="page-heading"><div><span className="overline">THE {cup.en} EXPERIENCE</span><h1>每一場，都有新的可能<span>。</span></h1></div><p><span className="online-dot" />陽光賽場開放中 <span className="divider">/</span> 每 2 分鐘一場</p></div>
