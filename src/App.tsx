  ...
  return <>
    ...
    <main className="app-main"><div className="page-heading"><div><span className="overline">THE {cup.en} EXPERIENCE</span><h1>{m.page.heading[0]}<span>{m.page.heading[1]}</span></h1></div><p><span className="online-dot" />{m.page.open(local.venue)} <span className="divider">/</span> {m.page.cadence}</p></div>
      <div className="game-layout"><section className="main-column">
        ...
        <RaceStage onReady={setGameReady} game={game} now={now} muted={muted} expanded={expanded} onCollapse={() => setExpanded(false)} chatFeed={phone && <StageChatFeed messages={chat.messages} now={now} />} celebration={phase === 'result' && result && result.payout > 0 ? <div className="payout-celebration" key={`payout-${game.round}`} role="status"><div className="payout-sparks" aria-hidden="true">{Array.from({ length: 12 }, (_, i) => <i key={i} style={{ '--spark': i } as CSSProperties} />)}</div><span className="payout-kicker">WIN!{m.payout.kicker && ` · ${m.payout.kicker}`}</span><strong><Coins size={24} />+{n(result.payout)}</strong><span>{m.payout.note}</span><small>{m.payout.net} {result.payout - result.stake >= 0 ? '+' : ''}{n(result.payout - result.stake)}</small></div> : null} betsPanel={<section aria-label={m.tickets.label} className={`stage-bets placed-bets ${phase === 'result' && result && result.payout > 0 ? 'has-payout' : ''}`}><div className="ticket-strip-heading"><h3>{m.tickets.label} <span>{game.bets.length}</span></h3><b>{n(totalStake)}<Coins size={12} /></b></div>{game.bets.length === 0 ? <p className="empty-bets">{m.tickets.empty}</p> : <ul>{game.bets.map(b => <li key={`${b.id}-${phase}`} className={phase === 'result' && result && wins(b.pick, result.winner, field) ? 'winning-bet' : ''}><span>{pickLabel(b.pick)}<small>{n(b.amount)} × {price(odds(b.pick, field))}</small></span>{phase === 'result' && result ? <b className={wins(b.pick, result.winner, field) ? 'ticket-payout' : 'muted'}>{wins(b.pick, result.winner, field) ? <><span><Check size={10} />{m.tickets.won}</span><strong>+{n(Math.round(b.amount * odds(b.pick, field)))}</strong><small>{m.tickets.includesStake}</small></> : m.tickets.lost}</b> : <button disabled={locked} onClick={() => void cancel(b.id)} aria-label={m.tickets.cancel(pickLabel(b.pick))}><X size={14} /></button>}</li>)}</ul>}</section>} notification={message && <div key={message} className="toast" role="status" aria-live="polite"><span>{message}</span><button aria-label={m.tickets.closeToast} onClick={() => setMessage('')}><X size={15} /></button></div>}>{dockOpen ? <div className="stage-betting"><button className="dock-toggle" onClick={() => setDockOpen(false)} aria-expanded="true">{m.dock.collapse}<ChevronDown size={14} /></button><section className="markets"><div className="section-heading"><h2><span className="section-number">01</span>{m.dock.heading}</h2><span>{locked ? <><LockKeyhole size={13} />{m.dock.closed}</> : m.dock.fair}</span></div>
