...
func verify_podium() -> void:
	...
	assert(podium.runners.size()==3,"Only the top three horses belong on the 3D podium")
	...
	assert(podium.runners.size()==3,"A new round must replace the podium without duplicates")
	podium.animate(1.0,.016)
	print("PASS: only the actual top three on stage, rank heights and next-round reset")
	podium.queue_free()
	quit()
